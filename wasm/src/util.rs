...
pub fn vis(input: &Input, out: &Output, turn: usize) -> (i64, String, String) {
    let v = &out.out[..turn];
    let (score, state, (a, p1, p2)) = compute_score_details(input, out.start, &v);
    let n2 = (input.n * input.n) as f64;
    let w = 600 / input.n;
    let h = 600 / input.n;
    let W = w * input.n;
    let H = h * input.n;
    ...
    for i in 0..input.n {
        for j in 0..input.n {
            doc = doc.add(parts::rect(
                w * j,
                h * i,
                w,
                h,
                &parts::color(a[i][j] as f64 / n2),
                1.0,
            ));
            doc = doc.add(parts::txt(
                w * j + w / 2,
                h * i + h / 2,
                &a[i][j].to_string(),
                w / 2,
    ...
    doc = doc.add(parts::line(0, 0, W, 0, "black", 2));
    doc = doc.add(parts::line(0, 0, 0, H, "black", 2));
    doc = doc.add(parts::line(W, 0, W, H, "black", 2));
    doc = doc.add(parts::line(0, H, W, H, "black", 2));
    for i in 0..input.n {
        for j in 0..input.n - 1 {
            if input.vs[i][j] == '1' {
                doc = doc.add(parts::line(
                    w * j + w,
                    h * i,
                    w * j + w,
                    h * i + h,
    ...
    doc = doc
        .add(parts::cir(p1.1 * w + w / 2, p1.0 * h + h / 2, w / 4, "red"))
        .add(parts::cir(
            p2.1 * w + w / 2,
            p2.0 * h + h / 2,
            w / 4,
